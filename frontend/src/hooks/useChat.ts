...
export function useChat() {
  ...
  const handleFeedbackTextSubmit = useCallback(
  ...
  const handleExplanationFlow = useCallback(() => {
    ...
    setMessages((prev) => [
      ...prev,
      {
        id: userMsgId,
        role: "user",
        content: "Pode me explicar isso passo a passo?",
      },
    ]);
    ...
    setTimeout(() => {
      ...
      const explanation =
        mockExplanations.find(
          (e) => e.competencyCode === classification?.competencyCode,
        ) || mockExplanations[0];
  ...
  const handleQuestionFlow = useCallback(() => {
    ...
    setMessages((prev) => [
      ...prev,
      {
        id: userMsgId,
        role: "user",
        content: "Gostaria de resolver uma questão sobre isso.",
      },
    ]);
    setIsAiThinking(true);
    ...
    setTimeout(() => {
      ...
      const question =
        mockQuestions.find(
          (q) => q.competencyCode === classification?.competencyCode,
        ) || mockQuestions[0];
    ...
    (value: string) => {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "user",
          content: answerLabel
            ? `Vou apostar na alternativa: **${answerLabel}**`
            : "Vou apostar nessa alternativa!",
        },
      ]);
  ...
  const handleTipFlow = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const tipSet =
        mockTipSets.find(
          (t) => t.competencyCode === classification?.competencyCode,
        ) || mockTipSets[0];
      ...
      try {
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            role: "ai",
            content:
              "Ops, tive um problema ao processar. Pode tentar novamente?",
          },
        ]);
      } finally {
        setIsAiThinking(false);
